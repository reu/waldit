...
RSpec.describe Waldit do
  ["memory", "persisted"].each do |strategy|
    context "with #{strategy} strategy" do
      around do |example|
        original = Waldit.large_transaction_threshold
        Waldit.large_transaction_threshold = strategy == "memory" ? 10_000 : 0
        example.run
      ensure
        Waldit.large_transaction_threshold = original
      end
      ...
      it "audits a simple insert" do
        ...
        assert_equal "1", audit.new["name"]
        assert_empty audit.old
      ensure
        Waldit.model.delete_all
        record&.delete
      end
      ...
      it "doesn't audit anything when only ignored columns change" do
        record = Record.create(name: "1")
      ...
      it "doesn't audit anything when the record is created and immediately deleted on the same transaction" do
        watcher = Waldit::Watcher.new
        replication = create_testing_wal_replication(watcher)

        Waldit.model.transaction do
          Record.create(name: "1").delete
        end
      ...
      context "with diff storage policy" do
        around do |example|
          original = Waldit.store_changes
          Waldit.store_changes = :diff
          example.run
        ensure
          Waldit.store_changes = original
        end

        it "always stores new for inserts" do
          watcher = Waldit::Watcher.new
          replication = create_testing_wal_replication(watcher)
        ...
        it "always stores old for deletes" do
          record = Record.create(name: "1")
        ...
        it "stores diffs for updates" do
          record = Record.create(name: "1")

          watcher = Waldit::Watcher.new
          replication = create_testing_wal_replication(watcher)

          Waldit.model.transaction { record.update(name: "2") }
          ...
          assert_equal "update", audit.action
          assert_equal({ "name" => ["1", "2"] }, audit.diff)
          assert_nil audit[:old]
          assert_nil audit[:new]
          assert_equal({ "name" => "1" }, audit.old)
          assert_equal({ "name" => "2" }, audit.new)
        ensure
          Waldit.model.delete_all
          record&.delete
        end

        it "doesn't audit anything when the record ends up unchanged" do
          record = Record.create(name: "1")
